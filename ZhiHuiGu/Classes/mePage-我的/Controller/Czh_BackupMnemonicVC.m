...
#pragma mark
    ...
    for (int i = 0; i < _zjcArr.count; i++) {
        ...
        CGFloat picY = margin + (LABELH + 5) * row;
        
        UIButton *cityButton = [UIButton buttonWithType:UIButtonTypeCustom];
        cityButton.frame = CGRectMake(picX, CGRectGetMaxY(tsLabel.frame) + 20 + picY, LABELW, LABELH);
        ...
        cityButton.titleLabel.font = [UIFont systemFontOfSize:15.f];
    ...
    Czh_LoginBtn *ZJCNextBtn = [[Czh_LoginBtn alloc] initWithFrame:CGRectMake(20,Main_Screen_Height - 80, Main_Screen_Width - 40, 44)];
...
- (void)ZJCNextBtnClick{
    NSString *zjcToArrString = [self.zjcToArr componentsJoinedByString:@" "];
    CZHLog(@"zjcToArrString-----%@",zjcToArrString);
    if ([_mnemonicStr isEqualToString:zjcToArrString]) {
